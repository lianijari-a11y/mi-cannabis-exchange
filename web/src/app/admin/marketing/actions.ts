...
export async function scheduleTextAction(id: string, text: string, scheduledForIso: string) {
  const session = await requireRole("admin");
  try {
    await scheduleTextForLead(id, text, new Date(scheduledForIso), { role: "admin", id: session.user.id });
  } catch (err) {
...
export async function sendEmailAction(id: string, subject: string, body: string) {
  const session = await requireRole("admin");
  const result = await sendEmailToLead(id, subject, body, session.user.id, { role: "admin", id: session.user.id });
  revalidatePath(PATH);
  return result;
}
...
export async function scheduleEmailAction(id: string, subject: string, body: string, scheduledForIso: string) {
  const session = await requireRole("admin");
  try {
    await scheduleEmailForLead(id, subject, body, new Date(scheduledForIso), { role: "admin", id: session.user.id });
  } catch (err) {
    return { ok: false as const, error: err instanceof Error ? err.message : "Something went wrong." };
  }
  revalidatePath(PATH);
